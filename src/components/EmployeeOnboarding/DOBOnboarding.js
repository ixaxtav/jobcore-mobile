...
import { H1, H3, H4, Container, Title, Header, Body, Content, List, Button, Text, FooterTab, Footer, Input, Label, Item, DatePicker} from 'native-base';
import {
  REGISTER_ROUTE,
  FORGOT_ROUTE,
  APP_ROUTE,
...
class DOBOnboarding extends Component {
  ...
  componentDidMount() {
    ...
    this.accountStoreError = accountStore.subscribe('AccountStoreError', (err) =>
        ...
        {(t) => (
          ...
          <FooterTab>
            {moment().diff(_userBirthDate, 'years') > 17 ? (
                <Button style={{ backgroundColor: 'black',  borderRadius: 0}} onPress={() =>{
              ...
              <Text style={{color: "white", fontSize: 18}}>To continue, enter a date</Text>
            </Button>
            )}  
